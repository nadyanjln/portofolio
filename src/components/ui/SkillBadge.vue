...
const levelColor = computed(() => {
  ...
  if (lvl === 'expert') {
    return currentProfile.value === 'raqwan'
      ...
      : 'bg-[#9E0402]/15 text-[#ff4d4d] border-[#9E0402]/30'
  }
  if (lvl === 'advanced') {
...
<template>
  ...
  >
    ...
    <span 
      ...
      :class="currentProfile === 'raqwan' ? 'bg-emerald-400' : 'bg-[#9E0402] dark:bg-[#ff4d4d]'"
    ></span>
    ...
    <span 
      ...
      class="text-[10px] font-mono-tag text-zinc-400 px-1.5 py-0.5 rounded bg-white/5 border border-white/5 shrink-0 hidden sm:inline-block"
    >
      {{ skill.category }}
    </span>
  </div>
